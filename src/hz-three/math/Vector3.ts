import { VNodeCall } from "@vue/compiler-core";
...
class Vector3 {
  ...
  applyAxisAngle(axis: any, angle: number) {
  ...
  projectOnPlane(planeNormal: any) {
    this._vector.copy(this).projectOnVector(planeNormal);

    return this.sub(this._vector);
  }
  ...
  reflect(normal: any) {
    ...
    return this.sub(
      this._vector.copy(normal).multiplyScalar(2 * this.dot(normal))
    );
  }
  ...
  setFromSphericalCoords(radius: any, phi: any, theta: any) {
  ...
  setFromCylindricalCoords(radius: any, theta: any, y: any) {
  ...
  setFromMatrixColumn(m: any, index: any) {
    return this.fromArray(m.elements, index * 4);
  }

  setFromMatrix3Column(m: any, index: any) {
  ...
  fromBufferAttribute(attribute: any, index: any, offset: any) {
    if (offset !== undefined) {
      console.warn(
